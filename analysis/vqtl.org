...
* Read the data
  ...
  #+BEGIN_SRC ipython
    def load_data(proj_dir):
      counts = pd.read_table(os.path.join(proj_dir, 'data', 'scqtl-counts.txt.gz'), index_col=0)
...
* ZINB model
  ...
  #+BEGIN_SRC ipython
    ...
    def model0(counts, onehot, stoch_samples=10, **kwargs):
      ...
      dropout (1 x n) is pooled across genes
      mean (p x m) is pooled across cells
      dispersion (p x m) is pooled across cells
