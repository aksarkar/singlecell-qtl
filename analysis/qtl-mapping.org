...
* Implementation
  ...
  #+BEGIN_SRC ipython
    def qqplot(qtls):
      plt.clf()
      plt.gcf().set_size_inches(6, 6)
      plt.scatter(-np.log10(np.arange(1, 1 + qtls.shape[0]) / qtls.shape[0]), -np.log10(qtls['p_adjust']), s=1, c='k')
...
** Test validity of approximate permutation test
   ...
   #+BEGIN_SRC sh :eval never-export :dir /scratch/midway2/aksarkar/singlecell/geuvadis
    ...
    #!/bin/bash
    plink --memory 2000 --geno 0.01 --maf 0.05 --keep-fam /scratch/midway2/aksarkar/singlecell/geuvadisgeuvadis-subset.txt --vcf /project/compbio/geuvadis/genotypes/GEUVADIS.chr1.PH1PH2_465.IMPFRQFILT_BIALLELIC_PH.annotv2.genotypes.vcf.gz --recode vcf-iid --out geuvadis-chr1
...
** Reproduce bulk eQTL calls
   ...
   *Important notes:*
   ...
   #+BEGIN_SRC ipython
     ...
     write_pheno_file(bulk_expr.iloc[:,3:], gene_info, '/scratch/midway2/aksarkar/singlecell/reproduce-yang/test.bed', holdout=False)
   #+END_SRC
...
** Recall bulk eQTLs from log TPM
   ...
   #+CALL: qtltools(pheno="bulk", geno="/scratch/midway2/aksarkar/singlecell/reproduce-yang/yri-dosages.vcf.gz", op="--window 100000 --permute 10000") :dir /scratch/midway2/aksarkar/singlecell/recall-bulk

   #+RESULTS:
   : Submitted batch job 44548801
...
* Write out the QTLs
  ...
  #+BEGIN_SRC ipython
    with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
      bulk = pd.read_table('/scratch/midway2/aksarkar/singlecell/reproduce-yang/fastqtl_qqnorm_RNAseq_run.fixed.txt.gz', index_col=3)
      (bulk.iloc[:,3:]
       .rename(index=lambda x: x.split('.')[0], columns=lambda x: 'NA{}'.format(x))
       .reset_index()
       .rename(columns={'ID': 'gene'})
       .melt(id_vars='gene', var_name='ind')
       .to_sql('bulk', conn, index=False, if_exists='replace'))
