...
* Mean-QTL calling
  ...
  Compare the mean QTLs to the bulk RNA-Seq QTLs.

  #+BEGIN_SRC ipython
    def parse_bulk_qtl_record(row):
      gene, _ = row[0].split('.')
      id_, *_ = row[5].split('.')
      return pd.Series({'gene': gene, 'id': id_, 'beta': row.iloc[-3], 'p_beta': row.iloc[-1]})
  ...
  #+BEGIN_SRC ipython :results output html
    print(mean_qtls
          .merge(gene_info, left_on='gene', right_index=True)[['gene', 'name', 'id', 'beta', 'p_beta']]
          .merge(bulk_qtls, on=['gene', 'id'], how='inner')[['gene', 'name', 'id', 'beta_x', 'beta_y', 'p_beta_x', 'p_beta_y']]
  ...
  Count how many genes have a SNP with an estimated effect size (regardless of
  significance) in both single cell and bulk.

  #+BEGIN_SRC ipython
    joined = (mean_qtls.merge(bulk_qtls, on='gene', how='inner')[['gene', 'id_x', 'id_y', 'beta_x', 'beta_y']]
              .fillna('')
              .query('id_x != "" and id_y != ""'))
    joined.shape[0]
  #+END_SRC
  ...
  For genes which have a SNP with an estimated effect size in both single cell
  and bulk (not necessarily the same), count how many times the signs of the
  estimated effects are consistent with LD between the two SNPs.
...
* Test bulk eQTLs in the single cell data
  ...
  #+BEGIN_SRC ipython
    def parse_perm_bulk_qtl_record(row):
      ...
      return pd.Series({'gene': gene, 'id': id_, 'chr': chr_, 'pos': pos, 'p_beta': row.iloc[-1]})
  ...
  Test each bulk QTL-gene pair in the single cell data.

  #+BEGIN_SRC ipython
    def parse_vcf(record):
      geno = np.ma.masked_equal([sum(float(h) for h in g.split('/')) if '.' not in g else -1 for g in record[9:]], -1)
      geno -= geno.mean()
      return pd.Series(geno.filled(0))
  ...
  #+BEGIN_SRC ipython :async t
    ...
    for (_, x), (name, y) in zip(X.iterrows(), Y.iterrows()):
      beta, rss, *_ = np.linalg.lstsq(x.values.reshape(-1, 1), y.values.ravel(), rcond=None)
      sigma2 = rss / y.shape[0]
      se = sigma2 / np.inner(x.values, x.values)
      pval = _sf(np.square(beta[0] / se))
      result.append({'gene': name, 'beta': beta[0], 'p_sc': pval})
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    merged = (bulk_qtls.merge(pd.DataFrame.from_dict(result), on='gene', suffixes=['_bulk', '_sc'])
              .set_index('gene')
              .sort_values('p_beta')
              .reset_index(drop=True)[['id', 'p_beta', 'beta_bulk', 'p_sc', 'beta_sc']])
...
* Test single cell QTLs in the bulk data
