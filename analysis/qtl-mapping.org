...
* Introduction
  ...
  4. [[*Call dispersion-QTLs][We call dispersion QTLs]] in the scRNA-Seq data
...
* Call mean-QTLs in the single cell data
  ...
  #+BEGIN_SRC ipython
    with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
      log_mean = (pd.read_sql(
        """select gene, ind, case when llr < 1 then nb_log_mean else zinb2_log_mean end
        as log_mean from params where ind in 
...
* Call dispersion-QTLs
  ...
  #+BEGIN_SRC ipython
    with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
      log_disp = (pd.read_sql(
        """select gene, ind, case when llr < 1 then nb_log_disp else zinb2_log_disp end
        as log_disp from params where ind in 
        (select chip_id from annotation group by chip_id 
        having count(distinct sample) >= 50);""", conn)
                  .pivot(index='gene', columns='ind', values='log_disp'))
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    log_disp = log_disp.apply(qqnorm, axis=1)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :noweb no-export
    <<get-gene-info>>
    <<write-pheno-def>>
    write_pheno_file(log_disp, gene_info, holdout=True, output_file='/scratch/midway2/aksarkar/singlecell/scqtl-mapping/disp.bed')
  #+END_SRC
  ...
  #+CALL: tabix(input="disp.bed") :dir /scratch/midway2/aksarkar/singlecell/scqtl-mapping

  #+RESULTS:
  : Submitted batch job 44392072

  Run ~qtltools~.

  #+CALL: qtltools(pheno="disp", geno="/scratch/midway2/aksarkar/singlecell/reproduce-yang/yri-dosages.vcf.gz", op="--window 100000 --permute 10000") :dir /scratch/midway2/aksarkar/singlecell/scqtl-mapping

  #+RESULTS:
  : Submitted batch job 44392077

  Read the output.

  #+BEGIN_SRC ipython :noweb no-export
    <<read-qtltools-def>>
    log_disp_qtls = read_qtltools_output('scqtl-mapping/disp')
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :noweb no-export :ipyfile figure/qtl-mapping.org/disp-qtl-beta-approx.png
    <<plot-approx-perm-def>>
  ...
  :RESULTS:
  # Out[36]:
  [[file:figure/qtl-mapping.org/disp-qtl-beta-approx.png]]
  :END:

  Plot a QQ plot of the permutation p-values.

  #+BEGIN_SRC ipython :ipyfile figure/qtl-mapping.org/disp-qtl-qq.png
    plt.clf()
    plt.gcf().set_size_inches(6, 6)
    plt.scatter(-np.log10(np.arange(1, 1 + log_disp_qtls.shape[0]) / log_disp_qtls.shape[0]), -np.log10(log_disp_qtls['p_beta']), s=1, c='k')
    plt.plot([0, np.log10(log_disp_qtls.shape[0])], [0, np.log10(log_disp_qtls.shape[0])], c='r', ls='--')
    plt.xlabel('Expected $-\log_{10}(p)$')
    _ = plt.ylabel('Observed $-\log_{10}(p)$')
  #+END_SRC
  ...
  :RESULTS:
  # Out[37]:
  [[file:figure/qtl-mapping.org/disp-qtl-qq.png]]
  :END:
  ...
  #+BEGIN_SRC ipython :noweb no-export
    <<r-wrappers>>
    log_disp_qtls['fdr_pass'] = bh(log_disp_qtls['p_beta']) < 0.1
    log_disp_qtls['fdr_pass'].sum()
  #+END_SRC
...
* Call variance-QTLs from sample variances
  ...
  #+CALL: qtltools(pheno="var", geno="/scratch/midway2/aksarkar/singlecell/reproduce-yang/yri-dosages.vcf.gz", op="--window 100000 --permute 10000") :dir /scratch/midway2/aksarkar/singlecell/scqtl-mapping
  ...
  #+BEGIN_SRC ipython :noweb no-export :ipyfile figure/qtl-mapping.org/var-qtl-beta-approx.png
    <<plot-approx-perm-def>>
  ...
  :RESULTS:
  # Out[19]:
  [[file:figure/qtl-mapping.org/var-qtl-beta-approx.png]]
  :END:

  Plot a QQ plot of the permutation p-values.

  #+BEGIN_SRC ipython :ipyfile figure/qtl-mapping.org/var-qtl-qq.png
    plt.clf()
    plt.gcf().set_size_inches(6, 6)
    plt.scatter(-np.log10(np.arange(1, 1 + var_qtls.shape[0]) / var_qtls.shape[0]), -np.log10(var_qtls['p_beta']), s=1, c='k')
    plt.plot([0, np.log10(var_qtls.shape[0])], [0, np.log10(var_qtls.shape[0])], c='r', ls='--')
    plt.xlabel('Expected $-\log_{10}(p)$')
    _ = plt.ylabel('Observed $-\log_{10}(p)$')
  #+END_SRC
  ...
  :RESULTS:
  # Out[20]:
  [[file:figure/qtl-mapping.org/var-qtl-qq.png]]
  :END:
...
* Call CV-QTLs from sample moments
  ...
  #+BEGIN_SRC ipython
    cv = umi.groupby(annotations['chip_id'].values, axis=1).agg(lambda x: x.std(axis=1) / x.mean(axis=1)).dropna()
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :noweb no-export
    <<r-wrappers>>
    cv = var.loc[:,keep_inds].transform(lambda x: (x - x.mean()) / x.std(), axis=1).apply(qqnorm, axis=0)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :noweb no-export :ipyfile figure/qtl-mapping.org/cv-qtl-beta-approx.png
    <<plot-approx-perm-def>>
  ...
  #+BEGIN_SRC ipython :ipyfile figure/qtl-mapping.org/cv-qtl-qq.png
    plt.clf()
    plt.gcf().set_size_inches(6, 6)
    plt.scatter(-np.log10(np.arange(1, 1 + cv_qtls.shape[0]) / cv_qtls.shape[0]), -np.log10(cv_qtls['p_beta']), s=1, c='k')
    plt.plot([0, np.log10(cv_qtls.shape[0])], [0, np.log10(cv_qtls.shape[0])], c='r', ls='--')
    plt.xlabel('Expected $-\log_{10}(p)$')
    _ = plt.ylabel('Observed $-\log_{10}(p)$')
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    with sqlite3.connect('/project2/mstephens/aksarkar/projects/singlecell-qtl/browser/browser.db') as conn:
      X, Y = extract_qtl_gene_pair(cv_qtls[cv_qtls['fdr_pass']], var, dosages='/scratch/midway2/aksarkar/singlecell/reproduce-yang/YRI_SNPs_2_IPSC.txt.gen.gz')
      X.reset_index().melt(id_vars='gene', var_name='ind').to_sql('cv_qtl_geno', conn, index=False, if_exists='replace')
      conn.execute('create index ix_cv_qtl_geno on cv_qtl_geno(gene, ind);')
      Y.reset_index().melt(id_vars='gene', var_name='ind').to_sql('var', conn, index=False, if_exists='replace')
      conn.execute('create index ix_var on var(gene, ind);')
      cv_qtls[['gene', 'id', 'beta', 'p_beta', 'fdr_pass']].to_sql('cv_qtls', conn, index=False, if_exists='replace')
...
* Call Fano factor-QTLs from sample moments
  ...
  #+BEGIN_SRC ipython
    fano = umi.groupby(annotations['chip_id'].values, axis=1).agg(lambda x: x.var(axis=1) / x.mean(axis=1)).dropna()
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :noweb no-export
    <<r-wrappers>>
    fano = var.loc[:,keep_inds].transform(lambda x: (x - x.mean()) / x.std(), axis=1).apply(qqnorm, axis=0)
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :noweb no-export :ipyfile figure/qtl-mapping.org/fano-qtl-beta-approx.png
    <<plot-approx-perm-def>>
  ...
  #+BEGIN_SRC ipython :ipyfile figure/qtl-mapping.org/fano-qtl-qq.png
    plt.clf()
    plt.gcf().set_size_inches(6, 6)
    plt.scatter(-np.log10(np.arange(1, 1 + fano_qtls.shape[0]) / fano_qtls.shape[0]), -np.log10(fano_qtls['p_beta']), s=1, c='k')
    plt.plot([0, np.log10(fano_qtls.shape[0])], [0, np.log10(fano_qtls.shape[0])], c='r', ls='--')
    plt.xlabel('Expected $-\log_{10}(p)$')
    _ = plt.ylabel('Observed $-\log_{10}(p)$')
  #+END_SRC
