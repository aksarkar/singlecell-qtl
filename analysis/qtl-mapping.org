...
* Mean-QTL calling
  ...
  #+BEGIN_SRC ipython
    def parse_bulk_qtl_record(row):
      gene, transcript =  row['pid'].split('.')
      id_, chr_, pos = row['dummy2'].split('.')
      return pd.Series({'gene': gene, 'transcript': transcript, 'id': id_, 'chr': chr_, 'pos': pos, 'p_beta': row['bpval']})
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    bulk_qtls = (pd.read_table('/project2/gilad/singlecell-qtl/bulk/permutations.all.RNAseq_run.fixed.txt.gz.bh.txt', sep=' ')
  ...
  #+BEGIN_SRC ipython :results output html
    print(mean_qtls
          ...
          .merge(bulk_qtls, on=['gene', 'id'], how='inner')[['gene', 'name', 'id', 'beta', 'p_beta_x', 'p_beta_y']]
