...
* Introduction
  ...
  2. [[*Call mean-QTLs in the single cell data][We call mean QTLs]] in the scRNA-Seq data and show that approximate
     permutation test is not appropriate
...
* Call mean-QTLs in the single cell data
  ...
  #+BEGIN_SRC sh :var pheno="mean" :var partition="broadwl" :dir /scratch/midway2/aksarkar/singlecell :eval never-export 
    ...
    parallel -j28 qtltools cis --vcf /project2/gilad/singlecell-qtl/bulk/genotypes.vcf.gz --bed $pheno.bed.gz --permute 100000 --chunk {#} 100 --out $pheno-qtl.{#}.txt ::: $(seq 1 100)
  #+END_SRC

  #+RESULTS: qtltools
  : Submitted batch job 44093227
