...
* Test bulk eQTLs against single cell NB means

  Estimate a lower bound on the replication rate by:

  1. For each individual, for each gene, estimate a mean and dispersion in the
