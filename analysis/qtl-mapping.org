...
** Call mean-QTLs
   ...
   #+BEGIN_SRC ipython
     mean = mean.loc[:,keep_inds].transform(lambda x: (x - x.mean()) / x.std(), axis=1).apply(qqnorm, axis=0)
   #+END_SRC
   ...
   #+RESULTS:
   : Submitted batch job 44422720

   Run ~qtltools~.

   #+CALL: qtltools(pheno="sample-mean", geno="/scratch/midway2/aksarkar/singlecell/reproduce-yang/yri-dosages.vcf.gz", op="--window 100000 --permute 10000") :dir /scratch/midway2/aksarkar/singlecell/scqtl-mapping

   #+RESULTS:
   : Submitted batch job 44422722
...
** Call dispersion-QTLs
   ...
   #+BEGIN_SRC ipython
     fano = fano.loc[:,keep_inds].transform(lambda x: (x - x.mean()) / x.std(), axis=1).apply(qqnorm, axis=0)
   #+END_SRC
...
** Call \phi-QTLs
  ...
  #+BEGIN_SRC ipython
    log_phi = log_phi.apply(qqnorm, axis=1)
  #+END_SRC
