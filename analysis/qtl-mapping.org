...
* Mean-QTL calling
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell
    ...
    #!/bin/bash
    sort -k1,1 -k2,2n -k3,3n mean.bed | bgzip >mean.bed.gz
    tabix -p bed mean.bed.gz
  #+END_SRC
  ...
  #+BEGIN_SRC sh :dir /scratch/midway2/aksarkar/singlecell
    sbatch --partition=mstephens -c4 -J mean-qtl -o mean-qtl.log
    #!/bin/bash
    parallel -j4 qtltools cis --vcf /project2/gilad/singlecell-qtl/bulk/genotypes.vcf.gz --bed mean.bed.gz --nominal=0.01 --chunk {#} 100 --out mean-qtl.{#}.txt ::: $(seq 1 100)
  #+END_SRC
...
* Variance-QTL calling
  ...
  The fundamental relationship derived by [[http://genomebiology.com/2014/15/2/R29][Law et al 2014]] to transform to
  Gaussian phenotypes is:

  \[ \mathrm{variance} = \frac{1}{\mathrm{mean}} + \mathrm{dispersion} \]

  *TODO:* Do we need to worry about mean QTLs in LD with variance QTLs? Can we
  just plug in the mean? Should we use predictions from multivariate mean-QTL
  models? Should we fit a Gaussian process regression (per individual, or
  pooling all individuals)?
