...
* ML sanity check
  ...
  #+BEGIN_SRC ipython :results output
    with tf.Graph().as_default():
      ...
      log_rate = tf.get_variable('mean', [n, 1])
      bias = library_size + 6 * tf.log(10.)
      mean = tf.matmul(onehot, log_rate) + bias

      disp = tf.get_variable('disp', [n, 1])
      var = tf.exp(mean) + tf.matmul(onehot, tf.exp(disp))

      llik = .5 * tf.reduce_sum(-tf.log(var) - tf.log(tf.constant(2 * np.pi)) - tf.square(log_cpm - mean) / var) 

      opt = [log_rate, disp]

      train = tf.train.RMSPropOptimizer(learning_rate=5e-3).minimize(-llik)
...
* Voom component
  ...
  #+BEGIN_SRC ipython
    ...
    mean_effect = ed.models.Normal(loc=tf.zeros([p, 1]), scale=tf.ones([p, 1]))
    disp_effect = ed.models.Normal(loc=tf.zeros([n, 1]), scale=tf.ones([n, 1]))

    eta0 = tf.matmul(onehot, tf.matmul(genotypes, mean_effect))
    eta1 = tf.matmul(onehot, disp_effect)

    log_cpm = ed.models.Normal(
      loc=tf.exp(eta0),
      scale=tf.sqrt(tf.exp(-eta0) + tf.exp(eta1)))

    q_mean_effect = ed.models.Normal(loc=tf.Variable(tf.zeros([p, 1])), scale=tf.Variable(tf.ones([p, 1])))
    q_disp_effect = ed.models.Normal(loc=tf.Variable(tf.zeros([n, 1])), scale=tf.Variable(tf.ones([n, 1])))
    inf = ed.ReparameterizationKLqp(
      latent_vars={mean_effect: q_mean_effect, disp_effect: q_disp_effect},
      data={onehot: data['onehot'], log_cpm: data['log_cpm'], genotypes: data['genotypes']})
  #+END_SRC
  ...
  #+BEGIN_SRC ipython
    inf.run(n_samples=10, optimizer=tf.train.AdamOptimizer(learning_rate=1e-8))
  #+END_SRC
  ...
  #+BEGIN_SRC ipython :ipyfile figures/hurdle.org/voom-posterior-mean.png
    plt.clf()
    fig, ax = plt.subplots(2, 1, sharex=True)
    ax[0].errorbar(x=np.arange(len(res)), y=res['eta0_mean'], yerr=1.96 * np.sqrt(res['eta0_var']), marker='o')
    ax[0].set_xticks([])
    ax[0].set_ylabel('Latent mean')
    ax[1].errorbar(x=np.arange(len(res)), y=res['eta1_mean'], yerr=1.96 * np.sqrt(res['eta1_var']), marker='o')
    ax[1].set_ylabel('Latent variance')
  ...
  #+RESULTS:
  :RESULTS:
  [[file:figures/hurdle.org/voom-posterior-mean.png]]
  :END:
