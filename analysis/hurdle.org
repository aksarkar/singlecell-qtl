...
* Test case
  ...
  Plot the data, mean, and two standard deviations per individual:
...
* Model specification and inference
  ...
  #+BEGIN_SRC ipython
    ...
    disp_bias_scale = tf.Variable(tf.ones([1]))
    disp_bias = ed.models.NormalWithSoftplusScale(loc=tf.zeros([n, 1]), scale=disp_bias_scale)

    disp_effect_scale = tf.Variable(tf.ones([1]))
    ...
    q_ind_bias = ed.models.Normal(loc=tf.Variable(tf.random_normal([n, 1])), scale=tf.Variable(tf.ones([n, 1])))
    q_disp_bias = ed.models.Normal(loc=tf.Variable(tf.random_normal([n, 1])), scale=tf.Variable(tf.ones([n, 1])))
  ...
  #+RESULTS:
  :RESULTS:
  : 1000/1000 [100%] ██████████████████████████████ Elapsed: 6s | Loss: 2317.697
  :END:
  ...
  #+BEGIN_EXAMPLE
    ...
    0          1.807363        0.066596         -0.006338         0.05111
  #+END_EXAMPLE
  :END:
