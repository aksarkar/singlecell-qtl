...
* Voom component
  ...
  #+BEGIN_EXAMPLE
    ...
    0         -0.110934         0.004125         -0.002337         0.009983
  #+END_EXAMPLE
  :END:
  ...
  #+BEGIN_SRC ipython :ipyfile figure/voom.org/data.png
    ...
    plt.scatter(x=np.where(data['onehot'] == 1)[1] + np.random.normal(scale=0.1, size=n), y=data['log_cpm'], s=2)
  ...
  #+BEGIN_SRC ipython :ipyfile figure/voom.org/post-pred.png
    post_pred = ed.get_session().run(ed.copy(log_cpm, inf.latent_vars),
                         {
                           onehot: data['onehot'],
                           genotypes: data['genotypes'][:,0:1],
                           library_size: -data['normalizers'],
    ...
    plt.clf()
    plt.gcf().set_size_inches(8, 6)
    plt.scatter(x=np.where(data['onehot'] == 1)[1] + np.random.normal(scale=0.1, size=n), y=post_pred, s=2)
    plt.xlabel('Individual')
    plt.ylabel('$\log_2(CPM + 1)$')
    plt.gca().set_xticks([])
    plt.gcf()
