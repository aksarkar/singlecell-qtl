...
* NB estimation
  ...
  Following Hilbe 2012, the log likelihood is:
  ...
  simply minimizing the negative log likelihood using automatic differentation
  and gradient descent should give reasonable estimates.

* ZINB estimation

  As before, let \(r_{ijk}\) denote the number of molecules for individual
  \(i\), cell \(j\), gene \(k\). Let \(\pi_{ij}\) denote the probability of a
  "technical zero" (i.e., not arising from the negative-binomial).
...
* Gradient descent
  ...
  We implement the inference algorithm in tensorflow.
  ...
  As a first pass, perform stringent QC to avoid the sparsest genes and don't
  ...
  #+BEGIN_SRC ipython :tangle zinb.py
    mean, dispersion = fit(
      ...
      size_factor=annotations['mol_hs'].values.astype(np.float32).reshape(-1, 1),
      learning_rate=1e-2,
      max_epochs=2000)
  #+END_SRC
  ...
  #+RESULTS:
  : Submitted batch job 42190633

* Evaluate the estimated parameters

  #+BEGIN_SRC ipython
    log_mean = pd.read_table('mean.txt.gz', sep=' ', index_col=0)
    log_disp = pd.read_table('dispersion.txt.gz', sep=' ', index_col=0)
  #+END_SRC
