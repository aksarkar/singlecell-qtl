...
* Model specification and inference
  ...
  #+BEGIN_SRC ipython :exports none
    ...
    LogNormal = ds.TransformedDistribution(
  ...
  #+BEGIN_SRC ipython
    ...
    shared_dropout = Normal(tf.constant(0.), scale=tf.constant(1.))
    dropout = Normal(loc=tf.fill([n, p], shared_dropout), scale=tf.constant(1.))
    shared_mean = Normal(tf.constant(0.), tf.constant(1.))
    mean = LogNormal(loc=tf.fill([m, p], shared_mean), scale=tf.constant(1.))
    shared_dispersion = Normal(tf.constant(0.), tf.constant(1.))
    dispersion = LogNormal(loc=tf.fill([m, p], shared_mean), scale=tf.constant(1.))
