...
* Introduction
  ...
  1. [[*Serial algorithm][We implement CPU-based]] and [[*Parallel algorithm][GPU-based]] maximum likelihood estimation of the
     parameters assuming no dropout
  2. [[*Visualize the fitted models][We develop visual diagnostics]] of the fitted models to check goodness of fit
  3. [[*Compare NB to ZINB][We implement parameter estimation assuming dropout]] parameters per cell and
     per gene
...
* Read the data
  ...
  Use genes on odd chromosomes as "training" (to decide which confounders to
  correct for, and how to correct for them), and hold out genes on even
  chromosomes to use as validation.
  ...
  #+BEGIN_SRC ipython
    genes = (pd.read_table('/home/aksarkar/projects/singlecell-qtl/data/scqtl-genes.txt.gz')
             ...
             .query('chr != "hsY"')
             .query('chr != "hsMT"'))
    genes = genes[genes.apply(lambda x: bool(int(x['chr'][2:]) % 2), axis=1)]
    umi = umi.align(genes, join='inner', axis='index')[0]
  #+END_SRC
...
* Parallel algorithm
  ...
  #+NAME: zinb-impl
  #+BEGIN_SRC ipython :eval never :exports none
    def sigmoid(x):
...
* Compare NB to ZINB
  ...
  Estimate the parameters of the zero-inflated model.
