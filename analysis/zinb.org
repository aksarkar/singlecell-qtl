...
* ZINB model
  ...
  Hiding the technical details of the model implementation, we specify the
  model in [[http://edwardlib.org/][Edward]]:
    ...
    class _ZINB(ds.Distribution):
      ...
      def __init__(self, dropout, mean, dispersion, validate_args=False,
        ...
        with tf.name_scope(name, values=[dropout, mean, dispersion]):
          ...
          self._mean = tf.identity(mean, name="mean")
          self._dispersion = tf.identity(dispersion, name="dispersion")
  ...
  #+BEGIN_SRC ipython
    def model0(counts, onehot):
      ...
      with tf.Graph().as_default():
        ...
        dropout = ed.models.Normal(loc=tf.fill([n, p], shared_dropout),
                                   scale=tf.constant(1.))
        ...
        return ed.copy([q_dropout, q_mean, q_dispersion])
  #+END_SRC
