...
* Fit ZINB
  ...
  #+BEGIN_SRC sh :eval never-export :exports none
    sbatch --partition=gpu2 --gres=gpu:1 --mem=5G --time=120 --job-name=zinb --output=zinb.out --error=zinb.err
    #!/bin/bash
    source activate scqtl
    python zinb.py
  #+END_SRC
  ...
  #+RESULTS:
  : Submitted batch job 43243115

  #+BEGIN_SRC sh :exports none
    sacct -j 43243115 -o Elapsed,MaxRSS
  #+END_SRC
...
* Fit ZINB2
  ...
  #+BEGIN_SRC ipython :eval never :tangle zinb2.py :noweb tangle
    ...
    <<onehot-qc>>
    mean, dispersion, dropout = fit(
      umi=umi.head().values.T.astype(np.float32),
      onehot=onehot.values.astype(np.float32),
      size_factor=umi.agg(np.sum).astype(np.float32).values.reshape(-1, 1),
      gene_dropout=True,
      ind_dropout=True,
  ...
  #+RESULTS:
  : Submitted batch job 43215386

  #+BEGIN_SRC sh :exports none
    sacct -j 43215386 -o Elapsed,MaxRSS
  #+END_SRC
