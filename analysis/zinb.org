...
* Introduction
  ...
  5. [[*Fit ZINB2][We fit a model assumint dropout per individual per gene]]
  ...
  #+BEGIN_SRC ipython
    def estimated_cdf(x, mean, disp, size, onehot, gene_dropout=None, ind_dropout=None):
      ...
      if gene_dropout is not None:
        G = sp.expit(-gene_dropout.values.T) * G + sp.expit(gene_dropout.values.T)
      elif ind_dropout is not None:
        G = onehot.dot(sp.expit(-ind_dropout.values.T)) * G + onehot.dot(sp.expit(ind_dropout.values.T))
      return G
