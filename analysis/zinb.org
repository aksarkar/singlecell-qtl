...
* numpy/scipy implementation
  ...
  #+BEGIN_SRC ipython
    ...
    def sigmoid(x):
      ...
      return np.clip(sp.expit(x), -13, 13)

    def zinb(theta, x, size):
      theta, dropout = theta[:2], sp.expit(theta[2])
   ...
   #+BEGIN_SRC ipython :eval never :tangle test.py :noweb tangle
     <<zinb-imports>>
     import gzip
     import os
     ...
     def fit_gene(chunk):
       res = so.minimize(zinb, x0=[0, 0, 0], args=(chunk[:,0], chunk[:,1]), method='bfgs')
       if res.success:
         return res.x
       res = so.minimize(zinb, x0=[0, 0, 0], args=(chunk[:,0], chunk[:,1]), method='cg')
       if not res.success:
         print(res)
       return res.x
     ...
     with mp.Pool() as pool:
       ...
       breaks = breaks.ravel()[-1]
       res = pool.map(fit_gene, np.split(chunk[['value', 'size']].values, breaks))

     with gzip.open('/scratch/midway2/aksarkar/singlecell/result-{}.txt.gz'.format(os.getenv('SLURM_ARRAY_TASK_ID')), 'wt') as f:
       # The pool preserves the order of the output
       seen = set()
       for _, key in chunk[['gene', 'chip_id']].iterrows():
         key = tuple(key)
         if key not in seen:
           gene, ind = key
           log_mean, log_disp, logodds = res.pop(0)
           print(gene, ind, log_mean, log_disp, logodds, file=f)
           seen.add(key)
   #+END_SRC
...
* Fit ZINB
  ...
  #+BEGIN_SRC sh :eval never-export :exports none
    sbatch --partition=gpu2 --gres=gpu:1 --mem=16G --time=70 --job-name=zinb --output=zinb.out --error=zinb.err
    #!/bin/bash
    source activate scqtl
    python zinb.py
  #+END_SRC
