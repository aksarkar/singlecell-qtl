...
def update_gene(attr, old, new):
  ...
  with sqlite3.connect(db) as conn:
    global gene
    gene = next(conn.execute('select gene from var_qtls where var_qtls.gene == ?;', (gene_data.data['gene'][selected[0]],)))[0]
    print('Selected {}'.format(gene))
    ind_data.data = bokeh.models.ColumnDataSource.from_df(pd.read_sql(
      sql="""select var_qtl_geno.ind, var_qtl_geno.value as genotype, 
      ...
      var.value as var from var_qtl_geno, var, params where var_qtl_geno.gene == ?
      and var_qtl_geno.gene == var.gene and var.gene == params.gene and
      var_qtl_geno.ind == var.ind and var.ind == params.ind;""",
      params=(gene,),
      con=conn))
...
def init():
  with sqlite3.connect(db) as conn:
    gene_data.data = bokeh.models.ColumnDataSource.from_df(pd.read_sql(
      sql="""select gene, id, p_beta as p, beta from var_qtls where fdr_pass order by p_beta;""",
      con=conn))
